...
### 8.1 RAG + All AWS Services — Pilot Scope (MI + TX)

**LLM & Retrieval:**

| Component | Setup Cost | Monthly Cost (1,000 cases) |
|---|---|---|
| Bedrock KB — `state-law-kb` (~1.5K chunks) | $0 | ~$10 (OpenSearch Serverless minimum) |
| Bedrock KB — `payer-policy-kb` (~12K chunks) | $0 | ~$15 |
| Bedrock KB — `denial-outcomes-kb` (~5K chunks) | $0 | ~$10 |
| Titan V2 embeddings (initial + quarterly + per-case) | ~$3 | ~$3 |
| Claude Sonnet 4.6 inference (~6K in / 1.5K out) | $0 | ~$45 |
| Claude Haiku 4.5 (document classification, ~500 tokens/doc) | $0 | ~$8 |
| Cohere Rerank (optional, ~$1 per 1K queries) | $0 | ~$3 |

**PDF & Clinical Text Processing:**

| Component | Setup Cost | Monthly Cost (1,000 cases) |
|---|---|---|
| Amazon Textract (~5,000 docs/month, ~5 pages avg) | $0 | ~$250 (25K pages @ $1 per 100 pages) |
| Amazon Comprehend Medical (~5,000 docs, 10 entities avg) | $0 | ~$5 (50K units @ $0.0001 per unit) |

**Storage & Compute:**

| Component | Setup Cost | Monthly Cost (1,000 cases) |
|---|---|---|
| Amazon S3 (~50GB PDFs + logs/month) | $0 | ~$2 (storage @ $0.023/GB) |
| AWS Lambda (1000 cases × 10 invocations) | $0 | ~$0 (free tier covers it) |
| Amazon API Gateway (~10K API calls) | $0 | ~$0.05 |
| Amazon EventBridge (1000 triggers) | $0 | ~$0.50 |

**Security & Observability:**

| Component | Setup Cost | Monthly Cost (1,000 cases) |
|---|---|---|
| AWS KMS (1 CMK + operations) | $1 | ~$1.50 |
| CloudWatch Logs (~10GB logs/month) | $0 | ~$5 |
| CloudTrail (data events on HealthLake + S3) | $2 | ~$2 |
| AWS Secrets Manager (1 secret) | $0 | ~$0.40 |

**User Management & Health Data:**

| Component | Setup Cost | Monthly Cost (1,000 cases) |
|---|---|---|
| Amazon Cognito (1K monthly active users) | $0 | ~$0 (free tier) |
| AWS HealthLake (base capacity + API calls) | $100–200 | ~$200 (FHIR operations: ~100 writes/reads per case) |
...
| **LLM + RAG** | ~$3 | ~$84 |
| **PDF + Clinical** | $0 | ~$255 |
| **Storage + Compute** | $0 | ~$2.55 |
| **Security + Observability** | $3 | ~$9 |
| **User Mgmt + FHIR** | $100–200 | ~$200 |
| **🎯 TOTAL (Pilot)** | **~$106–206** | **~$550.55 / 1,000 cases** |

---

### Key Cost Insights:

1. **Textract is the biggest cost** (~$250/mo) — because scanned PDFs are common in medical records. If all PDFs are digital/searchable, this drops to ~$5/mo.

2. **HealthLake is second** (~$200/mo) — but this is HIPAA-compliant structured storage, non-negotiable for PHI.

3. **LLM + RAG is surprisingly cheap** (~$84/mo) — vector search and generation are the tiny slice of the bill.

4. **At scale (50 states):**
   - Textract scales linearly with document volume (assume 2x) → ~$500/mo
   - HealthLake stays ~$200/mo (doesn't scale with states, only with patient volume)
   - Bedrock KB storage increases ~3x → ~$45/mo additional
   - **Total ≈ $1,250/month at 50-state nationwide scale**
